...
export const uploadKidImage = async (file, nombre) => {
  const storageRef = ref(storage, `kids/${nombre}_${Date.now()}`);
  await uploadBytes(storageRef, file);
  return await getDownloadURL(storageRef);
};

// Guarda un niño en Firestore
const registerKid = async ({ nombre, edad, genero, imageUrl }) => {
  try {
    const data = {
      nombre,
      edad,
